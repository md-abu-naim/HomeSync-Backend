...
var createUser = async (payload) => {
  ...
  const tamplatePath = path3.join(process.cwd(), "src/app/tamplates/user-registation-otp.ejs");
...
var verifyUserEmail = async (payload) => {
  ...
  if (isUserExists?.isEmailVerified) {
    throw new AppError(httpStatus3.CONFLICT, "User already verified");
  }
  const otpKey = `user-registation-otp:${email}`;
  ...
  if (!redisUserData) {
    throw new AppError(httpStatus3.NOT_FOUND, "User Does not exists");
  }
  ...
  const tamplatePath = path3.join(process.cwd(), "src/app/tamplates/welcome-email.ejs");
  const html = await ejs.renderFile(tamplatePath, {
...
var loginUser = async (payload) => {
  ...
  if (user.password === null && user.googleId !== null) {
    throw new AppError(httpStatus3.CONFLICT, "User already has account registered with google. Try to google login");
  }
...
var googleLoginIntoDB = async (payload) => {
  ...
  if (!googleIdTokenPayload.email) {
    throw new AppError(httpStatus3.BAD_REQUEST, "Email not fount");
  }
  if (!googleIdTokenPayload.name) {
    throw new AppError(httpStatus3.BAD_REQUEST, "Name not fount");
  }
  const isUserExists = await prisma.user.findUnique({
    where: {
...
var refreshToken2 = catchAsync(async (req, res) => {
  if (!req.cookies.refreshToken) {
    throw new AppError(httpStatus4.BAD_REQUEST, "Refresh token is missing");
  }
...
var registerUserValidationSchema = z.object({
  ...
  password: z.string().min(6).regex(/[A-Z]/, { message: "Add an uppercase letter" }).regex(/[a-z]/, { message: "Add a lowercase letter" }).regex(/[0-9]/, { message: "Add a number" }),
...
var verifyUserEmailValidationSchema = z.object({
  email: z.string({
    message: "Email is required"
  }).trim().toLowerCase().email("Invalid email address format"),
  otp: z.string({
...
var auth = (...requiredRoles) => {
  return catchAsync(async (req, res, next) => {
    ...
    if (!user) {
      throw new AppError(httpStatus6.NOT_FOUND, "User not found. Please log in again.");
    }
    if (user.status === "BLOCKED") {
      throw new AppError(httpStatus6.FORBIDDEN, "Your account has been blocked. Please contact support.");
    }
    req.user = {
      email,
      name,
...
var getMe2 = catchAsync(async (req, res) => {
  const user = req.user;
  if (!user) {
    throw new AppError(httpStatus8.BAD_REQUEST, "User information is missing in the request");
  }
...
var updateUserValidationSchema = z2.object({
  name: z2.string().trim().min(2, "Name must be at least 2 characters long").optional(),
  imageUrl: z2.string().trim().url("Invalid image URL").optional(),
  imagePublicId: z2.string().trim().optional(),
  tenant: z2.object({
    phone: z2.string().trim().optional(),
    occupation: z2.string().trim().optional(),
    bio: z2.string().trim().optional(),
    preferredLocation: z2.string().trim().optional(),
    maxBudget: z2.number().positive("Maximum budget must be greater than 0").optional(),
    minBudget: z2.number().positive("Minimum budget must be greater than 0").optional()
  }).optional()
});
...
var createProperty = async (payload, userId, image) => {
  ...
  if (!owner) {
    throw new AppError(httpStatus9.NOT_FOUND, "Owner Profile Not Found");
  }
  const ImagesLink = await new Promise((resolve, reject) => {
    cloudinary.uploader.upload_stream(
      ...
      async (error, result) => {
        ...
        if (!result) {
          return reject(new AppError(httpStatus9.INTERNAL_SERVER_ERROR, "No Result Renturned"));
        }
        resolve(result);
      }
    ).end(image?.buffer);
...
var createPropertyValidationSchema = z3.object({
  ...
  totalRooms: z3.coerce.number({ message: "TotalRooms must be a number" }).optional()
});
var updatePropertyValidationSchema = z3.object({
  title: z3.string().trim().min(3, "Title must be at least 3 characters long").optional(),
  description: z3.string().trim().optional(),
  propertyType: z3.enum([
    PropertyType.HOUSE,
    PropertyType.APARTMENT,
  ...
  address: z3.string().trim().min(5, "Address must be at least 5 characters long").optional(),
  city: z3.string().trim().optional(),
  area: z3.string().trim().optional(),
  latitude: z3.coerce.number({
    message: "Latitude must be a number"
...
var updateRoomValidationSchema = z4.object({
  roomNumber: z4.string().trim().min(1, "Room number cannot be empty").optional(),
  title: z4.string().trim().optional(),
  description: z4.string().trim().optional(),
  roomType: z4.enum([
    RoomType.SINGLE,
  ...
  amount: z4.coerce.number({ message: "Amount must be a number" }).positive("Amount must be greater than 0").optional(),
  securityDeposit: z4.coerce.number({ message: "Security deposit must be a number" }).nonnegative("Security deposit cannot be negative").optional(),
  capacity: z4.coerce.number({ message: "Capacity must be a number" }).int("Capacity must be an integer").positive("Capacity must be greater than 0").optional()
});
var updateRoomAvailabilityValidationSchema = z4.object({
  availability: z4.enum([
...
import z5 from "zod";
var createPreferenceValidationSchema = z5.object({
  preferredGender: z5.enum([
    Gender.MALE,
    Gender.FEMALE,
    Gender.OTHER
  ], {
  ...
  minBudget: z5.coerce.number({
    message: "Minimum budget must be a number"
  }).nonnegative("Minimum budget cannot be negative").optional(),
  maxBudget: z5.coerce.number({
    message: "Maximum budget must be a number"
  }).positive("Maximum budget must be greater than 0").optional(),
  preferredLocation: z5.string().trim().optional(),
  smokingAllowed: z5.boolean().optional(),
  petsAllowed: z5.boolean().optional()
});
var updatePreferenceValidationSchema = z5.object({
  preferredGender: z5.enum([
    Gender.MALE,
    Gender.FEMALE,
    Gender.OTHER
...
import z6 from "zod";
var createApplicationValidationSchema = z6.object({
  roomId: z6.string({ message: "Room ID is required" }).min(1, "Room ID is required"),
  message: z6.string().trim().optional(),
  proposedMoveIn: z6.string().datetime("Invalid move-in date").optional()
});
var updateApplicationStatusValidationSchema = z6.object({
  status: z6.enum(
    [
...
var createApplication = async (userId, payload) => {
  ...
  if (!room) {
    throw new AppError(httpStatus17.NOT_FOUND, "Room Not Found");
  }
...
var getBkashIdToken = async () => {
  try {
    ...
    if (!response.ok) {
      throw new AppError(httpStatus19.BAD_GATEWAY, "Bkash Access Token Grant Failed");
    }
    ...
    throw new AppError(httpStatus19.BAD_GATEWAY, error.message);
  }
};
...
var bkashCallback = async (query) => {
  const transactionResult = await prisma.$transaction(
    async (tx) => {
      ...
      if (!paymentId) {
        throw new AppError(httpStatus20.BAD_REQUEST, "Payment Id Missing");
      }
      const status = query.status;
      if (!status) {
        throw new AppError(httpStatus20.BAD_REQUEST, "Payment Status is Missing");
      }
      const bkashIdToken = await getBkashIdToken();
      if (!bkashIdToken) {
        throw new AppError(httpStatus20.BAD_GATEWAY, "No Bkash Access Token Found!");
      }
...
var createPaymentValidationSchema = z7.object({
  rentalId: z7.string({ message: "Rental ID is required" }).min(1, "Rental ID is required")
});
...
app.get("/", async (req, res) => {
  res.status(httpStatus22.OK).json({
    success: true,
